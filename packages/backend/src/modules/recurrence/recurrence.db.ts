...
export function createRecurrenceTables(
  references: {
  ...
  const recurrence = table("recurrence", {
  ...
  const recurrenceRules = table("recurrence_rules", {
    id: text("id").primaryKey().$default(uuidv4),
    createdAt: integer("created_at", { mode: "timestamp" })
      .notNull()
      .$default(() => new Date()),
...
const recurrenceTables = createRecurrenceTables();

export const recurrence = recurrenceTables.recurrence;
export const recurrenceRules = recurrenceTables.recurrenceRules;
