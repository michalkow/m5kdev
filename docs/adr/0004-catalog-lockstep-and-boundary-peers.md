...
The version promise is lockstep with the framework release, not “apps may run a newer compatible minor.” `m5kdev update` advances managed catalog pins (local+upstream change is a conflict). Extra app-owned catalog keys stay. Doctor still errors on `CATALOG_VERSION_MISMATCH`.
